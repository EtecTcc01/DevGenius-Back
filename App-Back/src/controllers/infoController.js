...
import db from '../services/infoServices.js'

const routes = express.Router();

routes.post('/', async (request, response) => {
  const {userName, userDate, userSex, fullName} = request.body;
  
  try {
    await db.createInfo(userName, userDate, userSex, fullName);
...
routes.put('/', async (request, response) => {
  ...
  await db.updateInfo({userName, userDate, userSex, fullName});
